...
def combine(topic: str, source_url: str) -> str:
    return "Write a short, only slightly (around 10%) sarcastically funny in the style of Rick Riordan, and quantitative news story with lots of bolded numbers on '" + topic + "' with a bolded title that does not contain quotation marks nor slashes and has each word starting with a capital letter, multiple twists and turns, less than 50 words for each paragraph, surround bolded words with <b> and </b> so it translates to html but with no tags for the title, the first two sentences should concisely summarize the entire story, and end each paragraph and the title with a string '+++++' by summarizing this article: " + newspaper_text_extraction(source_url) + "\n Your goal is to keep the reader curious and guessing at what comes next at the end of each paragraph."
